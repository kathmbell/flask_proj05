...
def get_predict_page(): 
    ...
    if form.validate_on_submit(): #this makes sure your form has data 
        ...
        return flask.render_template('predictor.html',
                                    # mapmost_likely_class_name = mapvar, 
...
def get_clusters(text, salary_range):
    if request.method == 'POST':
        try:
            ...
            # text = data['text'] # receive data from input
            ...
            #   'text': text
            # })
            vec = tech_request_vec(text, tech_columns) # call function to create vector for model
            vec = [vec] # this many needed for the .predict
            cluster_id = kmodes_model.predict(vec) # predict on vec, returns predicted cluster id
            ...
            for country in cluster_info['country_level_info'].keys():
                for item in cluster_info['country_level_info'][country].items():
                    if item[0] == 'SalaryBinned':
                        ...
                        if response == True:
                            print(salary_bin, cluster_info['country_level_info'][country])
                            # output_response.append(cluster_info['country_level_info'][country])
                            output_response[country] = cluster_info['country_level_info'][country]
            ...
            #     columns=['CountryName', 'PercentOfRespondents'],
            #     key_on='feature.id',
            #     fill_color='YlGn',
            #     fill_opacity=0.7,
            #     line_opacity=0.2,
